...
for file in os.listdir(config_file_base_path):
    ...
    if file_ext != '.json' or filename == '../experiments_configs/base_config':
        continue
    config_files.append(new_config)

def execute_exp(config_file_path):
    config = read_experiment_config(config_file_path)
    subprocess.call('python -u old_main.py --config-file={} --mode=production --device=cpu --base-config=../experiments_configs/base_config.json | tee ../logs/{}.log'.format(config_file_path, config['name']), shell=True)
